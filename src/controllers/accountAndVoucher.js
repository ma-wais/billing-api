...
export const getCashVouchers = async (req, res) => {
  ...
  try {
    let cashVouchers = await CashVoucher.find(query).populate(
      "account",
      "accountName"
    );

    if (account) {
      cashVouchers = cashVouchers.filter(
        (voucher) =>
          voucher.account.accountName.toLowerCase() === account.toLowerCase()
      );
    }
...
export const getCashVoucher = async (req, res) => {
  try {
    const cashVoucher = await CashVoucher.findById(req.params.id).populate(
      "account",
      "accountName balance"
    );
...
export const deleteCashVoucher = async (req, res) => {
  try {
    const deletedCashVoucher = await CashVoucher.findByIdAndDelete(
      req.params.id
    );
...
export const updateCashVoucher = async (req, res) => {
  try {
    const updatedCashVoucher = await CashVoucher.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
export const getAccountBalances = async (req, res) => {
  try {
    const { showReceivable, showPayable, showExpenses, showTrading } =
      req.query;
    ...
    accounts.forEach((account) => {
      ...
      const totalPayments = cashVouchers
        .filter(
          (voucher) =>
            voucher.account._id.toString() === account._id.toString() &&
            voucher.type === "CashPayment"
        )
      ...
      const totalReceipts = cashVouchers
        .filter(
          (voucher) =>
            voucher.account._id.toString() === account._id.toString() &&
            voucher.type === "CashReceipt"
        )
        .reduce((sum, voucher) => sum + voucher.amount, 0);

      const balance =
        openingDebit - closingCredit + totalReceipts - totalPayments;

      if (balance > 0 && showReceivable === "true") {
        balances.receivable.accounts.push({
          accountName: account.accountName,
          balance,
        });
        balances.receivable.total += balance;
      } else if (balance < 0 && showPayable === "true") {
        balances.payable.accounts.push({
          accountName: account.accountName,
